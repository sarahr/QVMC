...
#define DENSITY false // store equilibrium walker distribution
#define PAIRCOR false // set "true" for pair correlation function
#define POSITION true // set "true" to save positions
...
void DMC::run_algo(int N, int N_therm, double del_t, double E_t, int samples,
    ...
    ofstream ofile4;
    ostringstream ost;
    ost << "paircor" << myrank << ".dat";
    ofile4.open(ost.str().c_str(), ios::out);
#endif  
#if POSITION
    ofstream ofile5;
    ostringstream ost;
    ost << "positionDMC" << myrank << ".dat";
    ...
    while (no_sample < samples) {
            ...
            for (int loop_p = 0; loop_p < num_alive; loop_p++) {
                ...
                if ((k % 1000) == 0) {
                   for (int h = 1; h < numpart; h++) {
                       for(int m = 0; m < h; m++){
                            ofile4 << QDot[loop_p]->Trial->Pos->r_int(m,h) << " ";

                       }
 
                    }
                    
                    ofile4 << green_branch << endl;
                }
        ...
        if ((k%500)==0)
            for (int part = 0; part < numpart; part++) {
                for (int m = 0; m < dim; m++) {
                    ofile5 << QDot[loop_p]->Trial->Pos->current(part, m) << " ";
                }
                 ofile5 << endl;
            }
#endif
